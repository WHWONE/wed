document.addEventListener("DOMContentLoaded", () => {
  ...
  // Safari fallback (MediaRecorder)
  let safariDest = audioCtx.createMediaStreamDestination();
  let safariRecorder = null;
  let safariChunks = [];
  let safariIsRecording = false;

  // Load worklet only if not Safari
  if (!isSafari) {
...
function shouldRest() {
  ...
  // Separate Audio Recording buttons
...
NOTES.forEach(n => {
  ...
  if (startAudioRecBtn && stopAudioRecBtn) {
...
if (humanizeToggle.checked && timingVariation > 0) {
  ...
  // ===== Unified Start/Stop (fallbacks internally) =====
  function startAudioRecording() {
    if (!piano) return;

    if (isSafari) {
      // Connect to speakers + recorder dest (idempotent)
      try { if (piano.output && piano.output.connect) piano.output.connect(audioCtx.destination); } catch (e) {}
      try { if (piano.output && piano.output.connect) piano.output.connect(safariDest); } catch (e) {}
      safariChunks = [];
      try {
        safariRecorder = new MediaRecorder(safariDest.stream);
      } catch (e) {
        console.error('Safari MediaRecorder init failed:', e);
        status.textContent = "❌ Recorder init failed.";
        return;
      }
      safariRecorder.ondataavailable = e => { if (e.data) safariChunks.push(e.data); };
      safariRecorder.onstop = () => {
        const blob = new Blob(safariChunks, { type: 'audio/wav' });
    ...
    if (!recNode) {
      try {
        ...
        if (piano.output && piano.output.connect) {
          try { piano.output.connect(recNode); } catch (e) {}
        }
      } catch (e) {
  ...
  function stopAudioRecording() {
    if (isSafari) {
      if (safariRecorder && safariIsRecording) {
        safariRecorder.stop();
        safariIsRecording = false;
    ...
    if (recNode) { try { recNode.port.postMessage({ command: 'stop' }); } catch (e) {} }

    // Merge buffers and download WAV
